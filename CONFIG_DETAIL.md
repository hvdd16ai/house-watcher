...
> 為什麼 591 的欄位名稱（`houseage`、`parking`）跟永慶/信義（`house_age`、`has_parking`）風格不一樣：591 的抓取邏輯是把 `HOUSE_591` 的 key 原封不動當成 API 參數送出去，所以欄位名稱就是 591 API 本身的參數名；永慶/信義則是程式自己組網址路徑，欄位名稱是另外取的好記名字，跟網址片段的對應關係見各自表格。

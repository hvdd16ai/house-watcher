...
- 各平台盡量對應官方篩選參數（總價/型態/房數/坪數/車位/屋齡等），伺服器端先篩過可減少要抓的頁數
- 內建去重機制，同一物件不會重複通知
- 自動偵測同一戶被不同仲介重複刊登的情況
- 已通知過的物件如果降價，會另外發一則「降價提醒」通知
- 支援多人同時接收 Telegram 通知
- 完全沒有新物件/降價時可選擇定期發送心跳通知，確認排程仍在正常運作
...
多人接收：`chat_id` 也可以填一個陣列，每個人都會收到通知：

```json
"chat_id": ["<Chat ID 1>", "<Chat ID 2>"]
```
...
| `TELEGRAM_CHAT_ID` | Chat ID（多人接收用逗號分隔，例如 `111,222`） |
...
| `min_rooms`（YUNGCHING／SINYI 個別設定） | 房數門檻，程式解析格局文字判斷、必定套用；另有選填的 `rooms` 欄位可額外套用平台原生房數篩選以減少要抓的頁數，詳見 [`CONFIG_DETAIL.md`](CONFIG_DETAIL.md) |
| `NOTIFY_DUPLICATES` | 疑似重複刊登物件是否推播通知，預設 `false` |
| `NOTIFY_PRICE_DROPS` | 已通知物件降價是否另外推播「降價提醒」，預設 `true`；價格一律會更新進資料庫，此設定只影響要不要推播 |
| `HEARTBEAT_ENABLED` / `HEARTBEAT_HOURS` | 完全沒有新物件/降價時，是否定期發送心跳通知確認排程存活；預設關閉，開啟後最短間隔預設 24 小時 |
...
**降價提醒沒有通知到某個物件**
- 降價偵測僅比對「這次執行時剛好被抓到」的物件，並非每次都會重新掃描全部已記錄物件；若降價後該物件的排序未被推回抓取範圍內，則不會被偵測到，屬已知限制
...
| 房數篩選 | 平台伺服器端（極少數情況存在誤判） | 程式端解析格局文字為主，另有選填的平台原生篩選可搭配 | 程式端解析格局文字為主，另有選填的平台原生篩選可搭配 |
...
- **降價偵測**：對這次抓到、且 key 已存在資料庫中的物件比對總價，比記錄的低即視為降價，發送獨立的「降價提醒」通知；價格一律更新進資料庫，`NOTIFY_PRICE_DROPS` 只影響是否推播。比對前會先確認地址／坪數／格局皆與資料庫紀錄相符才視為同一戶——591／永慶的物件編號在下架一段時間後可能被平台回收挪給全新物件，若欄位對不上則不算降價，避免誤把兩個不相關物件的價格拿來比較
- **多人接收**：Telegram 通知可同時發送給多個 chat_id（Python 版填陣列，GAS 版以逗號分隔）
- **心跳通知**：`HEARTBEAT_ENABLED` 開啟後，僅在該次執行完全沒有新物件/降價、且距上次心跳已達 `HEARTBEAT_HOURS` 時才發送，避免與一般通知重複或過於頻繁

...
**房數篩選**：`YUNGCHING` 目前仍由程式解析「格局」文字判斷房數（對應每組 target dict 中的 `min_rooms`），未直接使用上表新發現的 `_rmp` 參數——如需切換為使用平台原生房數篩選，需調整 `yungching.py` 的目標網址組裝邏輯。

> ⚠️ **`parse_url.py` 目前只讀取網址中的 `_c`（地區）片段**，上表其餘參數（`_price`、`_type`、`_rmp`、`_pin`、`_park`、`_age`）貼入網址解析工具後會被忽略、不會出現在輸出結果中。若貼入含有這些條件的完整搜尋網址，記得額外檢查是否有條件被工具漏掉；程式（`yungching.py`）目前也未實作發送這些條件到永慶伺服器。
...
**房數篩選**：與永慶相同，由程式解析「格局」文字判斷（`min_rooms`），信義平台無伺服器端房數篩選。
